  ...
  switch (message.type) {
    case "handshake": {
      ...
      return [createMessage("handshake_ack", {
        capabilities: ["tabs_snapshot", "request_tabs_snapshot", "bookmarks_snapshot", "history_batch", "activate_tab", "open_url"],
    ...
    case "tabs_snapshot": {
      ...
      await index.upsertTabs(tabs as never);
    ...
    case "bookmarks_snapshot": {
      ...
      await index.upsertBookmarks(bookmarks as never);
    ...
    case "command_result": {
      ...
      if (payload.commandId) {
        await commandQueue.complete(payload.commandId, {
          ...
          action: payload.action === "activate_tab" ? "activate_tab" : payload.action === "tabs_snapshot" ? "tabs_snapshot" : "open_url",
